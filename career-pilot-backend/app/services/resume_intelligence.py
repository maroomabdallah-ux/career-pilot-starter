...
def _question(section: str, item: dict, language: str = "en") -> str:
    ...
    if section == "experience":
        ...
        return (
            f"ما الذي عملت عليه بشكل أساسي في {company}؟"
            if ar
            else f"What did you mainly work on at {company}?"
        )
    if section == "projects":
        return (
            "ما المشكلة التي يحلها هذا المشروع؟" if ar else "What problem does this project solve?"
        )
    if section == "summary":
        return "ما نوع الدور الذي تستهدفه؟" if ar else "What type of role are you targeting?"
    if section == "education":
        return (
            "هل لديك تميّز أكاديمي موثق تود إضافته؟"
            if ar
            else "Do you have a verified academic distinction you would like to include?"
        )
    return (
        "هل توجد مهارات أخرى موثقة في ملفك تريد تضمينها؟"
        if ar
        else "Which verified profile skills are most relevant to your target role?"
    )
...
def _suggestion(
    ...
    reason: str,
    text: str | None = None,
    evidence: list[ResumeEvidence] | None = None,
...
def analyze_section(
    ...
    if section == "summary":
        ...
        if not _substantive(texts[0]):
            missing.append("distinctive professional focus")
        elif len(_words(texts[0])) > 80:
            issues.append(
                ResumeIssue(type="too_long", message="The summary is longer than it needs to be.")
            )
        else:
            strengths.append("The summary communicates a specific professional direction.")
    elif section in {"experience", "projects", "education"}:
        rows = content.get(section, [])
        if item_index is None:
            item_index = 0 if rows else None
        item = rows[item_index] if item_index is not None and item_index < len(rows) else {}
        if section == "experience":
            texts = item.get("bullets") or []
        else:
            texts = [item.get("description") or ""]
        if not texts or not any(_substantive(x) for x in texts):
            missing.append("specific work, contribution, or outcome")
        else:
            strengths.append("This section includes specific, usable detail.")
    else:
        texts = [x for g in content.get("skill_groups", []) for x in g.get("items", [])]
        if not texts:
            missing.append("verified skills")
        else:
            strengths.append("Skills are based on saved profile claims.")
    ...
    if generic:
        issues.append(
            ResumeIssue(
                type="too_vague",
                message="This content does not explain the work or contribution clearly.",
            )
        )
        suggestions.append(
            _suggestion(
    ...
    if relevant:
        suggestions.append(
            _suggestion(
                ...
                "CareerPilot found career context that may make this section more specific.",
                relevant[0],
                [
                    ResumeEvidence(
                        source_type="career_knowledge", domain=section, excerpt=relevant[0]
                    )
                ],
                True,
            )
    ...
    quality = (
        ...
        if missing
            ...
            "needs_improvement"
            if issues
            else ("strong" if strengths and len(texts) > 1 else "good")
        )
    )
    return ResumeSectionAnalysis(
        ...
        issues=issues,
        strengths=strengths,
        supported_suggestions=suggestions,
...
def analyze_resume(
    content: dict, verified: dict, rag: list[str] | None = None
) -> ResumeAnalysisResponse:
    analyses = [analyze_section(content, verified, "summary", rag=rag)]
    for section in ("experience", "projects", "education"):
        analyses.extend(
            analyze_section(content, verified, section, i, rag)
            for i in range(len(content.get(section, [])))
        )
...
def apply_suggestion(
    content: dict, suggestion: ResumeSuggestion, edited_text: str | None = None
) -> dict:
    ...
    if suggestion.section == "summary":
        ...
        if suggestion.bullet_index is None:
            row.setdefault("bullets", []).append(text)
        else:
            row["bullets"][suggestion.bullet_index] = text
    elif suggestion.section == "projects":
        result["projects"][suggestion.item_index]["description"] = text
        ...
        raise ValueError("This suggestion type cannot be applied as text")
    return result


def validate_suggestion(
    suggestion: ResumeSuggestion, verified: dict, rag: list[str], user_answer: str | None = None
) -> list[str]:
    text = suggestion.suggestion or ""
    evidence_text = " ".join(
        [str(verified), *rag, user_answer or "", *(e.excerpt or "" for e in suggestion.evidence)]
    ).casefold()
    errors = []
    ...
    if (
        ...
        and not user_answer
        and not any(e.source_type == "profile" for e in suggestion.evidence)
    ):
