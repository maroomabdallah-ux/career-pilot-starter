...
def get_career_embeddings() -> OpenAIEmbeddings:
    if not settings.OPENAI_API_KEY:
        raise CareerKnowledgeConfigurationError("OPENAI_API_KEY is required for career knowledge retrieval.")
...
class CareerKnowledgeService:
    ...
    async def ingest_document(
        self, *, source_type: str, title: str, content: str, source_reference: str | None = None,
        ...
        document = await self.repository.create_document(CareerKnowledgeDocument(
            user_id=self.user_id, source_type=source_type, title=title, source_reference=source_reference
        ))
        await self.repository.create_chunks([
            CareerKnowledgeChunk(
                document_id=document.id, user_id=self.user_id, content=chunk, embedding=vector,
                metadata_json={**(metadata or {}), "source_type": source_type, "document_title": title},
                chunk_index=index,
            )
    ...
    async def retrieve(self, query: str, *, domain: str | None = None, limit: int = 5) -> list[CareerKnowledgeChunk]:
        embedding = await get_career_embeddings().aembed_query(query)
        return await self.repository.similarity_search(self.user_id, embedding, min(max(limit, 1), 6), domain)
