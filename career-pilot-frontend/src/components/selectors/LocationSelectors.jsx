...
export function CountryCombobox({value,onChange,label="Country"}){const [options,setOptions]=useState([]);const [error,setError]=useState("");useEffect(()=>{api().then(x=>x.getCountries()).then(setOptions).catch(()=>setError("Unable to load countries. Enter your country manually."))},[]);return <PremiumCombobox label={label} value={value} onChange={onChange} options={options} allowCustom placeholder="Search countries" error={error} getLabel={x=>typeof x==="string"?x:`${x.emoji||""} ${x.name}`} getKey={x=>typeof x==="string"?x:x.iso2}/>}
